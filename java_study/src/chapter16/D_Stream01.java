...
//	cf) Arrays.asList();
//	: 일반 배열을 ArrauList로 반환
//	>> 원소를 추가하는 메서드를 가지고 있지 않음, 사이즈를 바꿀수 없음
 	
//	## java.util.ArrayList와 다른 클래스

//	=== 특징 ===
//	- 컬렉션(Collection) 데이터를 간결하고 효율적으로 처리
//	- 데이터를 필터링, 매핑, 축소 등의 작업을 용이하게 수행하도록 도움줌
//	- 함수형 프로그래밍 기법과 함께 사용
...
public class D_Stream01 {
	public static void main(String[] args) {
		...
		//	== 스트림 API 기본 사용법 == 
		//	1. 스트림 생성
		List<String> names = Arrays.asList("손흥민", "황희찬", "황인범", "김민재");
		
		//	cf) .stream()으로 생성된 스트림은 일회성 사용만 가능!
		//	>> 한 번 사용한 스트림은 재사용 할 수 없음
		Stream<String> stream = names.stream();
		
		//	2. 중간 연산
		//	: 여러 개를 체인처럼 연결하여 사용 가능
		Stream<String> filteredStream = stream.filter(name -> name.startsWith("황"));
...
//		Stream<Integer> lengthStream = stream.map(String::length);
		...
		List<String> filteredNames = names.stream().filter(name -> name.startsWith("김")).collect(Collectors.toList());
		System.out.println("=============");
